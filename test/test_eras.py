"""Tests for ovos_date_parser.eras — AstroDate, year ranges, era resolution.
...
from ovos_date_parser import DateTimeResolution
from ovos_date_parser.eras import (AstroDate, ERAS, astro_year_range,
...
class TestAstroDateBasics(unittest.TestCase):
    ...
    def test_date_interop(self):
        ...
        d = AstroDate.from_date(datetime(1999, 12, 31, 23, 59))
        self.assertEqual((d.year, d.month, d.day), (1999, 12, 31))
        self.assertEqual(d.resolution, DateTimeResolution.DAY)
    ...
    def test_validation(self):
        ...
        with self.assertRaises(ValueError):
            AstroDate(2020, 0)
        with self.assertRaises(ValueError):
            AstroDate(2020, None, 5)      # day without month
        with self.assertRaises(ValueError):
    ...
    def test_str_iso8601_expanded(self):
        self.assertEqual(str(AstroDate(2020, 6, 15)), "2020-06-15")
        self.assertEqual(str(AstroDate(-4712, 1, 1)), "-4712-01-01")
        self.assertEqual(str(AstroDate(12000)), "+12000")
        self.assertEqual(str(AstroDate(50, 3)), "0050-03")
...
class TestAstroDateOrdering(unittest.TestCase):
    ...
    def test_eq_and_hash_are_astrodate_only(self):
        # == against date is False (not an error), and hashing is consistent
        # with AstroDate equality so dict/set use is safe
        self.assertEqual(AstroDate(2020, 1, 1), AstroDate(2020, 1, 1))
        self.assertNotEqual(AstroDate(2020, 1, 1), date(2020, 1, 1))
...
class TestResolveEra(unittest.TestCase):
    def test_before_christ(self):
        ...
        self.assertEqual(d, AstroDate(-2999, resolution=DateTimeResolution.YEAR))
    ...
    def test_anno_mundi(self):
        # AM 5786 spans 2025/26 CE; year-precision anchor 2025
        self.assertEqual(resolve_era("anno_mundi", 5786), date(2025, 1, 1))
        self.assertEqual(resolve_era("anno_mundi", 1), AstroDate(-3760))
